  ...
  <div class="ma-8">
    ...
    <v-data-table :headers="headers" :items="pageItems" item-key="id" items-per-page="20">
      ...
      <template v-slot:item.action="{ item }">
        <v-btn color="primary" @click="loadPagesData(3)">对比</v-btn>
      </template>
    </v-data-table>
  </div>
</template>
...
export default {
  ...
  data: () => ({
    headers: [
      { title: '名称', key: 'title', width: "10%" },
      { title: '帮助文档', key: 'doc', value: 'title', width: "10%", sortable: false },
  ...
  setup(props) {
    ...
    const pageItems = computed(() => {
      ...
      return _pageItems.value.filter(obj => obj.name.toLowerCase().includes(itemFilter.value.toLowerCase()));
    })
    function loadPagesData(ns) {
      return
      const reqUrl = API.pagelist
      if (props.ns != undefined) {
        reqUrl = API.pagelist + "?ns=" + props.ns
      }
      axios.get(reqUrl)
        .then(res => {
